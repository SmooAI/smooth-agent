...
//!
//! `smooth-operator-agent-core`'s [`KnowledgeChatRuntime`] proves the engine ↔ gateway
...
use smooth_operator_agent_core::adapter::{MessageQuery, StorageAdapter};
use smooth_operator_agent_core::domain::{Direction, Message as DomainMessage, MessageContent};
use smooth_operator_agent_core::tools::KnowledgeSearchTool;
