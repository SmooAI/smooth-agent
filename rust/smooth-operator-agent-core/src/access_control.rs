...
//! [`KnowledgeResult`](smooth_operator::KnowledgeResult) that carries only
...
/// [`KnowledgeBase`](smooth_operator::KnowledgeBase) and records document ACLs
...
mod tests {
    ...
    fn from_metadata_absent_is_none() {
        let doc = Document::new("c", "s", smooth_operator::DocumentType::Documentation);
