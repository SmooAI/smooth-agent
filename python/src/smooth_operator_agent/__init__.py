"""smooth_operator_agent — Python protocol types + native async client for the smooth-operator-agent
WebSocket protocol.

The protocol contract is defined by the language-neutral JSON Schemas in ``spec/``.
The generated pydantic models (:mod:`smooth_operator_agent._generated`) are committed so
consumers don't need the generator; :mod:`smooth_operator_agent.types` layers the ergonomic
