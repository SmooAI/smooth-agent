...
smooth-operator-agent makes a different bet:

- **Cloud-codable by default.** `npx smooth-operator-agent deploy` (SST) stands up an entire agent backend on AWS — API Gateway WebSockets + Lambda + DynamoDB + S3 Vectors — no servers to babysit. A Helm chart stands up the same thing on Kubernetes with Postgres + pgvector.
...
| **`smooth-operator-agent`** (this repo) | The **service** that turns the engine into a product — conversations, knowledge ingestion + retrieval, a tool catalog, the WebSocket protocol, and the AWS/k8s deploy paths. |

## Run it hosted

Don't want to operate it yourself? **[lom.smoo.ai](https://lom.smoo.ai)** runs smooth-operator-agent as a managed service.
