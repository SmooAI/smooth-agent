...
smooth-operator-agent ships **two first-class deploy paths** from one codebase, with the storage adapter as the seam that makes it possible. AWS-serverless is the default ("cloud-codable by default"); Kubernetes is the self-host path.
