...
**Rerank (`smooth_operator::rerank`, feature gap G8 — ✅ implemented).** After hybrid retrieval (dense ∪ sparse → RRF) the top-K can be **optionally** reordered by a sharper query↔candidate relevance model before it reaches the model's context. The `Reranker` trait (`rerank(query, candidates, top_k)`) has three impls: `NoopReranker` (identity — wiring it in changes nothing, which is what makes the stage opt-in), `LexicalReranker` (deterministic, network-free query-term-overlap / BM25-ish lexical score, offline-testable), and the production cross-encoder **`GatewayReranker`** in the **adapter** crate alongside `GatewayEmbedder`. `GatewayReranker` POSTs a Cohere/Voyage-style `{ model, query, documents, top_n }` to the gateway's `/v1/rerank` route (base + key from `SMOOAI_GATEWAY_URL` / `SMOOAI_GATEWAY_KEY`, same as the embedder), reads the returned `index → relevance_score`, reorders the original candidates by descending score, and truncates to `top_k`. On any failure (network / non-2xx / malformed / out-of-range index) it degrades to the **input order** truncated to `top_k` — a quality stage never panics or drops the turn — logging a `tracing::warn!`. It's generic over a `RerankBackend` seam so unit tests inject a stub and exercise reorder/truncate/error-fallback offline (mirrors `GithubSearchBackend`); the production backend is `HttpRerankBackend`. The reranker is wired into the `knowledge_search` tool behind `KnowledgeSearchTool::with_reranker(...)`: when set, the tool overfetches candidates (`limit × 4`) and reorders them; when unset (the default) behavior is unchanged. The server-side `build_reranker(RerankerConfig)` selector (mirrors `build_embedder`) chooses gateway-when-keyed / lexical / noop from the `SMOOTH_AGENT_RERANK` env (`gateway` | `lexical` | unset⇒off), returning `None` by default so retrieval is byte-for-byte unchanged unless rerank is explicitly enabled. Both the reference WS server and the lambda dispatch select the reranker through it and thread it onto `TurnRequest`.
...
- **OpenSearch Serverless k-NN** — powerful hybrid (vector + BM25 in one engine, closest to a Vespa-style engine), but higher floor cost and operational surface; offered as an opt-in backend for users who want managed hybrid in one place.
