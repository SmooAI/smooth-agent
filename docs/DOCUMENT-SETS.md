...
[access control](ACCESS-CONTROL.md)) and its `reader` takes an `AccessContext`
...
- [ACCESS-CONTROL.md](ACCESS-CONTROL.md) — the ACL layer this composes with (same
  over-fetch-then-filter pattern).
- [INGESTION.md](INGESTION.md) — the pull→chunk→embed→store pipeline that stamps the
  curation metadata.
- [STORAGE.md](STORAGE.md) — the `StorageAdapter` knowledge slice the stores wrap.
