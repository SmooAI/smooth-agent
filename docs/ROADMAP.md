...
- ✅ G1 ingestion framework (Connector/Chunker/pipeline + file/web). ✅ G3 document ACLs — **now enforced on the live chat path** (server + lambda read through `StorageAdapter::knowledge_for_access`), persisted durably (Postgres `acl` column filtered in SQL; DynamoDB `acl` attribute), `/ws` auth → `AccessContext` (fail closed to org-public), JWT `groups` claim wired; sibling cross-org admin leak fixed (org-keyed indexing-run + document-set registries). See [ACCESS-CONTROL.md](ACCESS-CONTROL.md). ✅ G4 LLM-judge evals. ✅ G8 rerank — **production `GatewayReranker` now shipped** (Cohere/Voyage `/v1/rerank` cross-encoder in the adapter crate; `build_reranker` selector + `SMOOTH_AGENT_RERANK` opt-in, default off; graceful input-order fallback on API error; wired into `knowledge_search` over-fetch→rerank→truncate on server + lambda). See [STORAGE.md](STORAGE.md). ✅ G5 widget Playwright e2e. ✅ G6 kind deploy smoke.
